...
**Each GTFS import gets its own PostgreSQL database** called `$GTFS_IMPORTER_DB_PREFIX_$unix_timestamp_$sha256_digest`. The importer keeps track of (the most recent) successful imports by – once an import has succeeded – writing the its DB name into a table `latest_successful_imports` within a "meta bookkeeping database".
